...
void main(void) {
    ...
    while (1) {
        ...
        if (PIND.1 == 0) { // Increment target
            delay_ms(50);
            while (PIND.1 == 0);
            target_position += STEP_DEG;
        }

        if (PIND.2 == 0) { // Decrement target
            delay_ms(50);
            while (PIND.2 == 0);
            target_position -= STEP_DEG;
        }
        ...
        if (motor_on) {
            ...
            // Stop if we reached or passed the target
            if ((direction == 0 && current_position >= target_position) ||
                (direction == 1 && current_position <= target_position)) {
                current_position = target_position; // Snap to target
                motor_on = 0;
            }
