...
char lcd_buffer[17];
int a;
unsigned char duty_cycle_percent;
...
while (1)
      {
        ...
        sprintf(lcd_buffer, "%3u ", adc_value);
        lcd_puts(lcd_buffer);

        a = adc_value *4;
        if (a > 1023) a = 1023;
        lcd_gotoxy(0,0);
        sprintf(lcd_buffer, "%4u", a);
        lcd_puts(lcd_buffer);

        OCR0 = adc_value;
        duty_cycle_percent = (unsigned char)((unsigned long)adc_value * 100 / 255);
        lcd_gotoxy(0,1);
        sprintf(lcd_buffer, "%3u", duty_cycle_percent);
        lcd_puts(lcd_buffer);
      }
}
